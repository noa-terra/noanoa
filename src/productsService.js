...
class ProductsService {
  ...
  getInventoryValueByCategory() {
    ...
    for (const category in values) {
      values[category] = Math.round(values[category] * 100) / 100;
    }
    return values;
  }
  ...
  getLowStockProducts(threshold = 10) {
    ...
    return this.products.filter((product) => product.stock < stockThreshold);
  }

  // Get products with stock above threshold
  getHighStockProducts(threshold = 50) {
    const stockThreshold = Number(threshold) || 50;
    if (stockThreshold < 0) {
      throw new ValidationError("Stock threshold must be non-negative");
    }
    return this.products.filter((product) => product.stock >= stockThreshold);
  ...
  getAveragePriceByCategory() {
    ...
    for (const product of this.products) {
      ...
      categoryData[category].total += product.price;
    ...
    for (const category in categoryData) {
      ...
      averages[category] = Math.round((data.total / data.count) * 100) / 100;
    }
